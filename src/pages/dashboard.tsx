...
import { useGameCount, useTipCount, useTrickCount, useRoqPlatformUserProfiles } from 'lib/roq';

import { TipListPage as ShowCaseList } from 'pages/tips';
...
function DashboardPage() {
  ...
  const tipStat = useTipCount();

  const trickStat = useTrickCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      title: 'Tips',
      result: tipStat?.data,
      isLoading: tipStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Tricks',
      result: trickStat?.data,
      isLoading: trickStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
